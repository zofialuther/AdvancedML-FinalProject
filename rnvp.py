...
class LinearRNVPUniform(nn.Module):
    """
    Main RNVP model, alternating affine coupling layers
    with permutations and/or batch normalization steps.
    """
    def __init__(self, input_dim, coupling_topology, flow_n=2, use_permutation=False,
        ...
        self.register_buffer('prior_min', torch.ones(input_dim)*-100)
        self.register_buffer('prior_max', torch.ones(input_dim)*100)
    ...
    @property
    def prior(self):
        return torch.distributions.Uniform(self.prior_min, self.prior_max)
